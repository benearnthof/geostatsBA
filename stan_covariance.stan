...
functions {
   ...
   */ 
  vector gp(vector[] x, real sdgp, vector lscale, vector zgp) { // add options
    int Dls = rows(lscale);
    int N = size(x);
    matrix[N, N] cov;
    if (Dls == 1) {
      // one dimensional or isotropic GP
      cov = cov_exp_quad(x, sdgp, lscale[1]);// add options
    } else {
      // multi-dimensional non-isotropic GP
      cov = cov_exp_quad(x[, 1], sdgp, lscale[1]);// add options
      for (d in 2:Dls) {
        cov = cov .* cov_exp_quad(x[, d], 1, lscale[d]);// add options
      }
    }
    for (n in 1:N) {
   ...
   */
  vector spd_cov_exp_quad(vector[] x, real sdgp, vector lscale) {// add options
    int NB = dims(x)[1];
    int D = dims(x)[2];
    int Dls = rows(lscale);
    vector[NB] out;
  ...
  vector gpa(matrix X, real sdgp, vector lscale, vector zgp, vector[] slambda) { 
    vector[cols(X)] diag_spd = sqrt(spd_cov_exp_quad(slambda, sdgp, lscale)); //add options
...
model {
  ...
  vector[N] mu = Intercept + rep_vector(0, N) + gp(Xgp_1, sdgp_1[1], lscale_1[1], zgp_1); //add options
  // priors including all constants
  target += student_t_lpdf(Intercept | 3, 8, 10);
  target += student_t_lpdf(sdgp_1 | 3, 0, 10)
    - 1 * student_t_lccdf(0 | 3, 0, 10);
  target += normal_lpdf(zgp_1 | 0, 1);
  target += inv_gamma_lpdf(lscale_1[1] | 0.866161, 0.004641);
